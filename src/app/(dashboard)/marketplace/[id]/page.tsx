...
export default function MarketplaceDetailPage() {
    ...
    const load = React.useCallback(() => {
        setLoading(true)
        fetch(`/api/marketplace/${params.id}?view=detail`)
            .then(async r => {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
                    ...
                    <div className="min-w-0 lg:border-r lg:border-border lg:pr-8">
                        ...
                        <div className="flex items-center gap-3 mb-5">
                            <Link href={`/profile/${listing.seller.id}`} className="w-10 h-10 rounded-full bg-accent/10 border border-accent/20 text-accent flex items-center justify-center font-bold text-sm shrink-0">
                                {listing.seller.name.slice(0, 1).toUpperCase()}
                            </Link>
                            <div className="min-w-0 flex-1">
                                <Link href={`/profile/${listing.seller.id}`} className="text-sm font-semibold text-text-primary hover:text-accent transition-colors">
