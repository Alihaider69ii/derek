  ...
  return (
    <div className="group flex flex-col gap-3 p-4 rounded-xl bg-bg-panel border border-border hover:border-[#2E5BFF]/30 transition-colors">
      ...
      <div className="flex items-center gap-2 pt-1 border-t border-border">
        <button onClick={handleCopy} className="flex items-center gap-1 text-xs text-text-secondary hover:text-text-primary transition-colors px-2 py-1 rounded-lg hover:bg-bg-hover">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bg-bg-panel border border-border w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-border">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg text-text-secondary hover:text-text-primary hover:bg-bg-hover">✕</button>
        </div>
        <div className="p-5 space-y-4">
          ...
          <button onClick={handleSave} disabled={!title.trim() || loading || done}
            ...
            style={{ background: done ? "linear-gradient(135deg,#22c55e,#16a34a)" : "linear-gradient(135deg,var(--accent),var(--accent-hover))" }}>
  ...
  if (notFound || !project) {
    return (
      <div className="flex flex-col h-full bg-bg-base items-center justify-center gap-4 p-6 text-center">
        ...
        <Link href="/projects"><button className="mt-2 px-5 py-2 rounded-full text-sm font-semibold text-white" style={{ background: "var(--accent)" }}>Back to Projects</button></Link>
      </div>
    )
  }
        ...
        <div className="flex gap-0">
          {(["chats", "prompts"] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-5 py-2.5 text-sm font-medium capitalize transition-colors border-b-2 -mb-px ${activeTab === tab ? "border-accent text-text-primary" : "border-transparent text-text-secondary hover:text-text-primary"}`}>
      ...
      <div className="flex-1 px-4 py-6">
        ...
        {activeTab === "prompts" && (
          <div className="pb-10">
            ...
            <div className="mb-4 flex items-start gap-3 bg-bg-input border border-border rounded-xl p-4">
              <span className="text-xl">💡</span>
              <p className="text-xs text-text-secondary leading-relaxed">
                Prompts are saved here when you click <strong className="text-text-primary">Save to Project</strong> on Derek&apos;s messages in the chat. Start a new chat above to generate prompts!
              </p>
            </div>
            ...
            {project.prompts?.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
                ...
                <button onClick={() => { setNewChatInput("Help me with " + project.name); setActiveTab("chats") }}
                  className="mt-1 px-5 py-2 rounded-full text-sm font-semibold text-white" style={{ background: "linear-gradient(135deg,var(--accent),var(--accent-hover))" }}>
                  Start a Chat
                </button>
              </div>
            )}
