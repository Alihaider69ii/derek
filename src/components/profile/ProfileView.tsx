...
// The public seller profile (Screen 7), rendered at the public /[username]
// URL — standalone, no dashboard sidebar, works for logged-out visitors.
// Takes the resolved user id as a prop — the caller (root [handle] route,
// or the legacy /profile/[id] redirect target) resolves username/id to it.
export function ProfileView({ id }: { id: string }) {
  ...
  if (notFound || !profile) {
    return (
      <div className="flex flex-col min-h-screen bg-bg-base items-center justify-center px-6 text-center">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-6">
          <div className="flex items-center gap-4">
            ...
            <div className="sm:hidden">
              ...
              <p className="text-xs text-white/70">@{profile.username} · {profile.sellerLabel}</p>
            </div>
          </div>
          <div className="flex-1 min-w-0">
            <p className="hidden sm:block font-bold text-white text-lg">{profile.name}</p>
            <p className="hidden sm:block text-xs text-white/70 mb-1.5">@{profile.username} · {profile.sellerLabel}</p>
